<template>
  <q-drawer v-model="show" side="right" bordered :width="700" class="bg-white hotel-drawer">
    <template v-if="hotel">
      ...
      <q-carousel v-model="currentSlide" swipeable animated arrows navigation infinite height="300px"
        class="hotel-carousel">
        <q-carousel-slide v-for="(image, index) in hotel.images" :key="index" :name="index" :img-src="image">
          <div class="absolute-top-right q-pa-sm">
            <q-btn round flat :icon="hotel.favorite ? 'favorite' : 'favorite_border'" color="white"
              class="bg-grey-9 bg-opacity-50" @click="toggleFavorite" />
          </div>
        </q-carousel-slide>
      </q-carousel>
      ...
      <div class="q-pa-md">
        <div class="row items-center q-mb-md">
          ...
          <div class="col-auto">
            <q-rating v-model="stars" :max="5" size="1.2em" color="amber" icon="star" readonly />
          </div>
        </div>
        ...
        <div class="q-mb-lg">
          ...
          <div class="row q-col-gutter-sm">
            <div v-for="amenity in hotel.amenities" :key="amenity.key" class="col-auto">
              <q-chip outline class="amenity-chip">
                <q-icon :name="getAmenityIcon(amenity.key)" size="18px" left />
                {{ amenity.label }}
              </q-chip>
            </div>
          </div>
          <q-btn flat color="primary" label="Mostrar todas as facilidades" class="q-mt-sm" no-caps />
        </div>
        ...
        <div class="row items-center justify-between">
          <div>
            ...
            <div class="text-h6">R$ {{ hotel.price }}</div>
          </div>
          <q-btn color="primary" label="Reservar agora" class="q-px-xl" no-caps @click="onBookNow" />
        </div>
      </div>
    </template>
...
import type { Hotel } from '../interfaces/interfaces'
